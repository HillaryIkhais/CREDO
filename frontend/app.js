...
const app = {
    ...
    captureFrame: async () => {
        ...
        if (app.state.isScanning) {
            setTimeout(app.captureFrame, 1000);
        }
    },
    ...
    handleVerdict: async (match) => {
        ...
        if (match.verdict === 'COUNTERFEIT') {
            ...
            iconWrap.innerHTML = `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>`;
    ...
    fetchDashboardData: async () => {
        try {
            ...
            document.getElementById('metric-fake').innerText = fake;
            document.getElementById('metric-rate').innerText = `${rate}%`;
            
            const tbody = document.getElementById('telemetry-table-body');
            if (total === 0) {
                tbody.innerHTML = `<tr><td colspan="5" class="empty-state">No telemetry data available.</td></tr>`;
                return;
            }
    ...
    renderHistory: () => {
        ...
        if (app.state.localHistory.length === 0) {
            container.innerHTML = `<div class="card empty-state">No scan history available.</div>`;
            return;
        }
            ...
            <div class="card history-item ${scan.verdict === 'COUNTERFEIT' ? 'fake' : 'safe'}">
                ...
                <div>
                    ${scan.verdict === 'COUNTERFEIT' 
                        ? `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--accent-red)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`
                        : `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--accent-green)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`
                    }
                </div>
            </div>
        `).join('');
